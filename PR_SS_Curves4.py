...
def binary_search(arr, target, low, high, func):
    best_val = 0
    while low <= high:
        mid = (low + high) // 2
        val = func(arr, mid)
...
def specificity_at_threshold(tn_cumsum, fp_cumsum, index):
    tn = tn_cumsum[index]
    fp = fp_cumsum[index]
    return tn / (tn + fp)
...
def sr_threshold(y_true: np.ndarray, y_prob: np.ndarray, min_specificity: float) -> Tuple[float, float]:
    ...
    positive_count = np.count_nonzero(y_true_sorted == 1)
    tn_cumsum = np.cumsum(y_true_sorted == 0)
    fp_cumsum = np.cumsum(y_true_sorted == 1)

    # Обновленный бинарный поиск
    threshold_index = binary_search(y_prob_sorted, min_specificity, 0, len(y_prob_sorted) - 1, lambda arr, idx: specificity_at_threshold(tn_cumsum, fp_cumsum, idx))
    threshold = y_prob_sorted[threshold_index]
    y_pred = y_prob_sorted >= threshold
    recall = np.count_nonzero((y_pred == 1) & (y_true_sorted == 1)) / positive_count
