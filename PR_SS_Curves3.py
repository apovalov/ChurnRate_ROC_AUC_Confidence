...
def sr_threshold(y_true: np.ndarray, y_prob: np.ndarray, min_specificity: float) -> Tuple[float, float]:
    ...
    threshold_index = binary_search(y_prob_sorted, min_specificity, lambda arr, idx: specificity_at_threshold(y_true_sorted, arr, idx))
    threshold = y_prob_sorted[threshold_index]
    y_pred = (y_prob_sorted >= threshold).astype(int)
    recall = np.count_nonzero((y_pred == 1) & (y_true_sorted == 1)) / positive_count  # использование предварительно вычисленного значения
